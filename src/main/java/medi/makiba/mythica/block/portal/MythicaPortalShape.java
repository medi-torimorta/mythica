...
import medi.makiba.mythica.registry.MythicaTags;


public class MythicaPortalShape {
	private static final int MIN_WIDTH = 1;
	public static final int MAX_WIDTH = 21;
	private static final int MIN_HEIGHT = 2;
	public static final int MAX_HEIGHT = 21;
	private static final BlockBehaviour.StatePredicate FRAME = (state, getter, pos) -> state.is(MythicaTags.Blocks.PORTAL_FRAME_BLOCKS);
	...
	public MythicaPortalShape(LevelAccessor level, BlockPos bottomLeftPos, Direction.Axis axis) {
		this.level = level;
		this.axis = axis;
	...
	private int getDistanceUntilTop(BlockPos.MutableBlockPos pos) {
		for (int i = 0; i < MAX_HEIGHT; ++i) {
			...
			for (int j = 0; j < this.width; ++j) {
				...
				if (blockstate.is(MythicaBlocks.MYTHICA_PORTAL.get())) {
	...
	private static boolean isEmpty(BlockState state) {
		return state.isAir() || state.is(MythicaBlocks.MYTHICA_PORTAL.get()) || state.is(BuiltInRegistries.BLOCK.getTag(MythicaTags.Blocks.PORTAL_REPLACEABLE).orElseThrow());
	}
	...
	public void createPortalBlocks() {
		BlockState blockstate = MythicaBlocks.MYTHICA_PORTAL.get().defaultBlockState().setValue(MythicaPortalBlock.AXIS, this.axis);
